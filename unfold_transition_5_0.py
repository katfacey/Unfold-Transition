...
class Set_Up_Fold(bpy.types.Operator):
    ...
    # tratar de limpiar rastros de procesados anteriores
        ...
        if "UnFold" in obj.modifiers:
            ...
            rig = arm.data
            try:
                scn.collection.objects.unlink(arm)
    ...
    # crear los huesos y vertex groups
        ...
        root.head = (0,0,1)
        root.select = True
        # vis = [False, True] + [False] * 30  <-- Removed: 'vis' is no longer used for EditBone
        ...
        for fb in S:
            ...
            b.select = False
            # b.layers = vis  <-- REMOVED: This is the problematic line
            b.parent = root
...
class Animate_Fold(bpy.types.Operator):
    ...
    def poll(cls, context):
        obj = bpy.context.object
        return (obj and obj.type == "ARMATURE") # and obj.is_visible(bpy.context.scene))
